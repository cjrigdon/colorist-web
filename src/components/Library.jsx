...
const Library = ({ user }) => {
  ...
  useEffect(() => {
    ...
    if (requestedSection === 'videos' || requestedSection === 'files' || requestedSection === 'playlists') {
  ...
  const fetchInspirations = useCallback(async (page = 1, append = false) => {
    try {
      ...
      const filters = {
        ...
        sort: section === 'videos' ? 'created_at' : 'title',
        sort_direction: section === 'videos' ? 'desc' : 'asc'
      };
      if (debouncedItemSearch) {
        filters.search = debouncedItemSearch;
      }
      if (tagFilterIds.length > 0) {
        filters.tags = tagFilterIds;
      }
      const response = await inspirationAPI.getAll(page, 25, filters);
  ...
  }, [section, tagFilterIds, debouncedItemSearch]);
  ...
  const gridItems = useMemo(() => {
    const apiItems = limitedInspirations;
    if (!isFreePlan) return apiItems;
    return apiItems.slice(0, FREE_PLAN_LIMIT);
  }, [limitedInspirations, isFreePlan]);
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {playlists.map((playlist) => {
                  ...
                  const fallbackCount = Number.isFinite(Number(playlist.preview_total))
                    ? Number(playlist.preview_total)
                    : 0;
                  const totalCount = videoCount || fallbackCount;
                  return (
                    <HoverableCard
                      key={playlist.id}
                      className="group cursor-pointer"
                      onClick={() => navigate(`/studio/inspiration/playlists/${playlist.id}`)}
                    >
                      ...
                      <div className="p-4 space-y-2">
                        <h3 className="font-semibold text-slate-800 line-clamp-2">{playlist.title}</h3>
                        ...
                        <div className="flex items-center justify-between pt-1">
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              ...
                              setShowPlaylistDeleteModal(true);
                            }}
