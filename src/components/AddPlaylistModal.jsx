...
const AddPlaylistModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      setError(null);
      await playlistsAPI.create({ playlist_url: playlistData.playlist_url });
      onSuccess();
      onClose();
      setPlaylistData({ playlist_url: '' });
    } catch (err) {
      console.error('Error creating playlist:', err);
      setError(err.data?.message || err.message || 'Failed to add playlist. Please check that the playlist URL is valid and the playlist is public.');
    } finally {
      setLoading(false);
    }
  };
